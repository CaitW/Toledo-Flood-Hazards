...
function createSymbols(data) {
    ...
    var symbols = mapSymbolGroup.selectAll('circle').data(data.features).enter().append('circle').attr('fill', colors[currentAttribute][0]).attr('stroke', function(d) {
        d["prev"] = 0;
        return 'white'
...
function init() {
    ...
    // Storm water Layer
    var stormWater = new L.tileLayer.wms(serviceURL + "/WMSServer?", {
        className: '3',
        layers: '1,stormwater',
        opacity: ($('[name="layerCheckboxes"]:eq(2)').is(':checked')) ? 1 : 0,
        position: "back"
    ...
    var chartMargin = {
        ...
        chartWidth = ($('#page-content-wrapper').width()) - (chartMargin.left + chartMargin.right),
        chartHeight = $("#chart").height() - chartMargin.top - chartMargin.bottom;
    var chartsvg = d3.select("#chart").append("svg").attr("id", "chartsvg").attr('width', chartWidth + chartMargin.left + chartMargin.right).attr('height', chartHeight + chartMargin.top + chartMargin.bottom).append("g").attr("class", "mainChart").attr("transform", "translate(" + chartMargin.left + "," + chartMargin.top + ")");
    ...
    var hazSVG = d3.select('#floodHazardsLegend').append('svg').attr('width', haz_w + haz_m.left + haz_m.right).attr('height', haz_h + haz_m.top + haz_m.bottom).attr('id', 'hazardLegendSVG').append('g').attr('transform', function() {
